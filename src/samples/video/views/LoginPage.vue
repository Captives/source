  ...
  <el-form class="login-panel" ref="form" :model="form" label-width="80px">
    <el-form-item label="房间">
      <el-select filterable v-model="form.region" placeholder="请选择活动区域" @change="selectHandler">
        <el-option v-for="item in form.list" :label="item.name" :value="item.id" :key="item.id"></el-option>
      </el-select>
    </el-form-item>
...
export default {
  ...
  methods: {
    login() {
      ...
      if (!region) {
        region = this.form.list[
          Math.floor(this.form.list.length * Math.random())
        ];
      }
